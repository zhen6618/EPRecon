...
class ScanNetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, id):
        ...
        pose_path = "pose/pose_" + "{:d}".format(id)
        cam_pose = np.loadtxt(os.path.join(self.data_path, self.scene, pose_path + ".txt"), delimiter=' ')

        # Read depth image and camera pose
        depth_path = "depth/depth_" + "{:d}".format(id)
        depth_im = cv2.imread(os.path.join(self.data_path, self.scene, depth_path + ".png"), -1).astype(
            np.float32)
        depth_im /= 1000.  # depth is saved in 16-bit PNG in millimeters
        depth_im[depth_im > self.max_depth] = 0

        # Read RGB image
        color_path = "color/color_" + "{:d}".format(id)
        color_image = cv2.cvtColor(cv2.imread(os.path.join(self.data_path, self.scene, color_path + ".jpg")),
                                   cv2.COLOR_BGR2RGB)
